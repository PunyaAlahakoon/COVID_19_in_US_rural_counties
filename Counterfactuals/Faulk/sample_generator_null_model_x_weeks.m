...
%x=days that the FOI should incraese 

%percentage to increase the tranmsiion 
function[prev,daily_c,inci]=sample_generator_null_model_x_weeks(times,par,N,count,perc,x)
s=12; %number of states 
%initial condition: one exposed person
ini=zeros(1,s); ini(4)=1; ini(1)=N-ini(4);

 t0=[times(1) times(1)+x times(2)]; 
 t2=[times(1)+x times(2) times(4)];
...
params(1:2,2:5)=0; %vaccination related parameters at fisr are zero
params(2:3,1:3)=perc*params(2:3,1:3); %increse of FOI happens after the first par set 
