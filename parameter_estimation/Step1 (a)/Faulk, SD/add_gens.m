...
for k=5:5
   ...
   if ismember(k,1:6) %SD; k=1:6
    ...
    params=load('para_smc2.mat');
    params=params.para_smc{1,k};
    %params=zeros(B,np);
    w= load('w_smc2.mat');
    w=w.w_smc;%store weights 
    w=w(:,k)';
    %es=zeros(G+1,4);
    ...
    dose2_paths0=zeros(length(cases),B);
     ...
     [params0(a,:),w0(a),rho_m(a,:),ag0(a),accepted_case_paths0(:,a),...
         dose1_paths0(:,a),dose2_paths0(:,a)]=abc_ind_step2(g,B,params,w,cases,deaths,vaccines,vacc_cum,e,count,times,N);
     end 
...
save('para_smc3.mat','para_smc')
save('case_paths3.mat',"accepted_case_paths")
save('dose1_paths3.mat',"accepted_dose1_paths")
save('dose2_paths3.mat',"accepted_dose2_paths")
save('w_sm3.mat','w_smc')
save('AG_smc3.mat','AG_smc')
save('sx3.mat','sx')
toc
