...
function[dists]=distance_metrics5(daily_c,inci,cases,deaths,vaccines,vaccine_cums,times)
        ...
        t=times(3):length(cases);  %take times times(3)+1 to check the second part of the vaccination 
        vx1=daily_c(t,4);
        %take cum sum 
        vm1=sum(vx1);
        %take moving average:
       ...
       % dists(5:6)=abs(inci(times(3)-1,4:5)-vaccine_cums);
        ...
        ox=daily_c(198:end,1);%generated 
        dists(5)=abs(sum(ox)-sum(cases(198:end)));
        
        %initial re-introduction specific values:
        oi=daily_c(1:41,1);%generated 
        dists(6)=abs(sum(oi)-sum(cases(1:41)));
