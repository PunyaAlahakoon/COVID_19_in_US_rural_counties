...
function[params0,w0,rho0,ag0,case_path,dose1_path,dose2_path]=abc_ind_step2(g,B,params,w,cases,deaths,vaccines,vaccine_cums,early_std,e,count,times,N)
    %model definition 
...
while(aa<2)
    ...
    if p1>0
        ...
        [dists]=distance_metrics5(daily_c,inci,cases,deaths,vaccines,vaccine_cums,times);
            ...
            %standardise the tolerances:
                std_dist=dists./early_std;
                %get the euclidiean distance:
                eu_d=sqrt(sum(std_dist.^2));
