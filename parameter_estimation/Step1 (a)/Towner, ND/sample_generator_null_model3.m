...
function[prev,daily_c,inci]=sample_generator_null_model3(times,par,N,count)
s=12; %number of states 
%initial condition: one exposed person
ini=zeros(1,s); ini(4)=1; ini(1)=N-ini(4);

 t0=[times(1) times(2) times(3)]; 
 t2=[times(2) times(3) times(4)];
...
nr=3;
params=repmat(par(1:15),nr,1);
params(1,2:5)=0; %vaccination related parameters at fisr are zero
params(2,4)=par(16); %start vaccination rates 
